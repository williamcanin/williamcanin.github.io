...
# Usage:        {% lbskill <content > | < size > %}
# Exemplo: {.% lbskill HTML|15px  %.}
# --------------------------------------------------------------------2
    ...
    class Main < Liquid::Tag
      ...
      def initialize(tag_name, input, tokens)
        ...
        if input =~ Syntax then
          ...
          raise "[lbskills.rb] Warning! Parameter for content not incorporated or incorrect."
        end
      end
      def render(context)
        ...
        %(<span style="font-size: #{fsize};" class="label label-default lb-skill">#{skill}</span>)
      end
...
Liquid::Template.register_tag('lbskill', Jekyll::LbSkills::Main)